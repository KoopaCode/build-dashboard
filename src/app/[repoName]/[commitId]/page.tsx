...
export default async function CommitPage({ 
  ...
  const commitDetails = await fetchCommitDetails(params.repoName, params.commitId);
  
  if (!plugin || !commitDetails) {
    notFound();
  }

  // Find the specific artifact for this commit
  const artifact = plugin.artifacts.find(
    art => art.workflowRun.headSha === params.commitId
  );
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-900 rounded-lg p-4">
              ...
              <div className="space-y-2">
                {commitDetails.files.map((file: CommitFile, index: number) => (
                  <div 
                    key={index}
